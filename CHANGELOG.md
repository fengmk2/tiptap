...
- f878c05: Nested lists exported to Markdown now keep their hierarchy when the file is read back by other Markdown tools.
- f878c05: Fix a denial-of-service risk where crafted block or inline Markdown attributes could consume excessive CPU and block the browser or server event loop.
- f878c05: Large transactions (for example a big paste with Link autolink enabled) are processed faster.
- f878c05: Prevent extra CSS declarations from being rendered from editor content.
- f878c05: Atom block directives (`:::name {…} :::`) indented by up to 3 spaces are now tokenized, matching CommonMark indentation rules for block constructs.
- f878c05: Numeric HTML entities (`&#39;`, `&#x27;`) are now decoded when parsing markdown, instead of showing up as literal text in the editor.
- f878c05: Fix freezes in framework-based node views on iOS and Android
- f878c05: Bind `parseMarkdown` and `renderMarkdown` to the configured extension so hooks can read `this.options` and `this.name` without an Editor.
- f878c05: Prevent untrusted HTML attributes from changing an object's prototype when merged with `mergeAttributes`.
- f878c05: Stop joinItemForward and joinItemBackward from joining across isolating nodes.
- Updated dependencies [f878c05]
...
- f878c05: Nested lists exported to Markdown now keep their hierarchy when the file is read back by other Markdown tools.
- f878c05: Parsing large Markdown documents that contain ordered lists is much faster. A 578KB document went from about 19s to about 160ms.
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
...
- f878c05: The Server AI Toolkit no longer adds `_hash` to inline nodes, including inline Image, Audio, YouTube and Twitch nodes.
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
...
- f878c05: Pasting a YouTube iframe without a `src` attribute no longer crashes the editor. The embed is kept without a source.
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
...
- f878c05: StarterKit no longer installs the deprecated `@tiptap/extension-dropcursor`, `@tiptap/extension-gapcursor`, `@tiptap/extension-list-item`, and `@tiptap/extension-list-keymap` packages. We will not publish further updates to these packages. StarterKit still includes their extensions, so you do not need to change your configuration.
...
- f878c05: Fix Markdown serialization of whitespace-only marked text.
- f878c05: Fix Markdown serialization of inline code containing backticks
- f878c05: Bind `parseMarkdown` and `renderMarkdown` to the configured extension so hooks can read `this.options` and `this.name` without an Editor.
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
...
- f878c05: Fixed a bug which allowed potentially unsafe color values being sent by other clients. Those unsafe colors received from collaboration users are now ignored.
- f878c05: Prevent extra CSS declarations from being rendered from editor content.
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
...
- f878c05: Align `selected` with ProseMirror node selections by default, expose text selections through `selectionInside`, and keep `selectedOnTextSelection` compatible.

#### Patch Changes

- f878c05: Reduce overhead in React node views on documents with many nodes.
- f878c05: Fix a TypeScript error (TS2694) in the shipped type declarations when `skipLibCheck` is turned off.
- f878c05: Fix Enter and Shift-Enter inside React node views on iOS and Android.
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
...
- f878c05: Bump `prosemirror-view` to `^1.42.3`, which fixes an XSS vulnerability where pasting crafted HTML could run arbitrary JavaScript (GHSA-c8x8-7fp4-3x9w).
...
- f878c05: Table cells exported to Markdown now escape literal pipe characters, so the cell content survives when the output is read back.
- f878c05: Multi-block table cells no longer leak a U+001F control character into Markdown.
- f878c05: Right-clicking a cell inside a multi-cell table selection no longer collapses that selection.
- f878c05: Prevent extra CSS declarations from being rendered from editor content.
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
...
- f878c05: Typing spaces after a link no longer extends the link, including multiple spaces inserted in one transaction. Formatting changes and undo/redo preserve existing linked whitespace.
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
- Updated dependencies [f878c05]
...
- f878c05: Vue 2 support is removed. `@tiptap/vue-3` is now `@tiptap/vue`, and `@tiptap/extension-drag-handle-vue-3` is now `@tiptap/extension-drag-handle-vue`.
